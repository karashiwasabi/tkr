...
import { getLocalDateString, parseBarcode, fetchProductMasterByBarcode } from 
'./utils.js';
    ...
    try 
{
  ...
  window.showNotification('GS1-128形式のバーコードではありません。', 'error');
    ...
    try {
        ...
        if 
(productMaster.yjCode !== currentYjCode) {
...
master.janPackInnerQty;
  ...
  calculationErrorByProduct[productCode] = '包装数量(内)未設定';
    ...
    if (lastLoadedDataCache 
&& lastLoadedDataCache.transactionLedger) {
...
1 : (tx.flag === 3 ? -1 : 0));
                                todayNetChangeByProduct[tx.janCode] = (todayNetChangeByProduct[tx.janCode] || 0) 
+ signedJanQty;
}
    ...
    document.querySelectorAll('.physical-stock-input').forEach(input => {
     ...
     if (calculationErrorByProduct[productCode]) {
            if (displaySpan) displaySpan.innerHTML = `<span 
class="status-error">${calculationErrorByProduct[productCode]}</span>`;
...
= calculatedPreviousDayStock.toFixed(2);
    });
}
...
const 
productCode = bottomRow.querySelector('[data-product-code]')?.dataset.productCode;
   ...
   if (target.classList.contains('register-inventory-btn')) {
    ...
    outputContainer.addEventListener('submit', 
(e) => {
        e.preventDefault();
        // ▼▼▼【ここから修正】のロジックを修正 ▼▼▼
