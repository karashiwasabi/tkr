...
${err.message}`, 'error');
   ...
   window.showLoading('製品情報を検索中...');
    try {
        // TKRの utils.js を使用
        const productMaster = 
await fetchProductMasterByBarcode(inputValue);
...
if (targetRow) {
            ...
            if (parsedData.expiryDate) { // YYYYMM 
形式
...
async function saveInventoryData() {
   ...
   const dateInput = document.getElementById('inventory-date');
    ...
    if 
(!confirm(`${dateInput.value}の棚卸データとして保存します。よろしいですか？`)) return;
    const inventoryData = {};
    const deadStockData = [];
    ...
    if (!cache || !cache.transactionLedger || cache.transactionLedger.length === 0) {
        window.showNotification('保存対象の品目データが見つかりません。', 
'error');
        return;
    }
