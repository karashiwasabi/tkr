...
export function renderTransactionTableHTML(records, customBody = null) {
 ...
 '<tr><td colspan="13">対象データがありません。</td></tr>' : records.map(rec => {
            ...
            const top = `<tr><td rowspan="2" class="col-action"></td>
                ...
                <td class="col-unitprice">${rec.unitPrice?.toFixed(4) ||
 ''}</td><td class="col-expiry">${rec.expiryDate || ''}</td><td class="col-wholesaler">${clientDisplayHtml}</td><td class="col-line">${rec.lineNumber || ''}</td></tr>`;
            
            const bottom = `<tr><td class="col-flag">${transactionTypeMap[rec.flag] || rec.flag}</td><td class="yj-jan-code col-jan">${rec.janCode ||
 ''}</td><td class="col-package">${rec.packageSpec || ''}</td><td class="col-maker">${rec.makerName || ''}</td>
                <td class="col-form">${rec.usageClassification ||
 ''}</td><td class="col-janqty">${rec.janQuantity?.toFixed(2) || ''}</td><td class="col-janpackqty">${rec.janPackUnitQty || ''}</td><td class="col-janunit">${rec.janUnitName || ''}</td>
                <td class="col-amount">${rec.subtotal?.toFixed(2) ||
 ''}</td><td class="col-lot">${rec.lotNumber || ''}</td><td class="col-receipt">${rec.receiptNumber || ''}</td><td class="col-ma">${rec.processFlagMA || ''}</td></tr>`;
// ▲▲▲【修正ここまで】▲▲▲
            return top + bottom;
        }).join('')}</tbody>`;
    }
