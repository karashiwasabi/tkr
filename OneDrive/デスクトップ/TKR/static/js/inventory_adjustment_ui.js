...
// ▼▼▼【ここから修正】yjCodeがnullでも各セクションを描画するように修正 ▼▼▼
export function generateFullHtml(data, cache) {
 
    // yjCode がない (品目が選択されていない) 場合の処理
    if (!data || !data.transactionLedger || data.transactionLedger.length === 0) {
        
        const yesterdaysTotal = 0; // 品目未選択時は 0
        const yjGroup = { productName: '(品目未選択)', yjCode: '---', packageLedgers: [], yjUnitName: '単位' }; // ダミーデータ

        // 「1. 全体サマリー」は非表示（メッセージのみ）
        ...
        // 「2. 棚卸入力」セクションは描画する
        // (品目依存の入力欄 [packageGroupsHtml] は空になるが、日付/バーコード欄は描画される)
