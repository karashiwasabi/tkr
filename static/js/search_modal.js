...
// ▼▼▼ 追加: 製品名入力と薬種コンテナ ▼▼▼
let modalProductNameInput, modalDrugTypesContainer;
// ▲▲▲ 追加ここまで ▲▲▲
let modalGs1Form;
...
async function performSearch() {
    ...
    // ▼▼▼ 追加: 製品名 ▼▼▼
    const productName = modalProductNameInput ? modalProductNameInput.value.trim() : '';
    ...
    // ▼▼▼ 追加: 薬種チェックボックスの収集 ▼▼▼
    const drugTypes = [];
    if (modalDrugTypesContainer) {
        const checkedBoxes = modalDrugTypesContainer.querySelectorAll('input[type="checkbox"]:checked');
        checkedBoxes.forEach(cb => {
            drugTypes.push(cb.value);
        });
    }
    ...
    if (drugTypes.length > 0) {
        params.append('drugTypes', drugTypes.join(','));
    }
    ...
    // 条件チェック（緩和: 薬種のみでも検索可能にする）
    if (!usageClass && drugTypes.length === 0 && !kanaName && !genericName && !productName && !shelfNumber) {
         window.showNotification('検索条件を入力してください。', 'warning');
        return;
    }
...
export function initSearchModal() {
  ...
  // ▼▼▼ 追加: DOM取得 ▼▼▼
  modalProductNameInput = document.getElementById('modal-search-product');
  modalDrugTypesContainer = document.getElementById('modal-search-drug-types');
  // ▲▲▲ 追加ここまで ▲▲▲
  ...
  addEnterListener(modalKanaInput);
  addEnterListener(modalGenericInput);
  addEnterListener(modalShelfInput);
  // ▼▼▼ 追加 ▼▼▼
  addEnterListener(modalProductNameInput);
  // ▲▲▲ 追加ここまで ▲▲▲
}
