...
/**
 * 実在庫入力行（1行分）のHTMLを生成
 * isPrimary=true の場合、theoreticalStock をプレースホルダーに表示し、valueは空にする。
 */
export function createFinalInputRow(master, deadStockRecord = null, isPrimary = false, theoreticalStock = '') {
    ...
    // 保存済みデータがある場合のみ値をセット。それ以外は空欄。
    let quantity = '';
    if (deadStockRecord) {
        quantity = deadStockRecord.stockQuantityJan;
    }

    // プレースホルダーに理論在庫(目安)を表示
    let quantityPlaceholder;
    if (isPrimary) {
        if (theoreticalStock !== '' && theoreticalStock !== null) {
            quantityPlaceholder = `目安: ${theoreticalStock}`;
        } else {
            quantityPlaceholder = '実在庫数を入力';
        }
    } else {
        quantityPlaceholder = 'ロット数量';
    }

    const expiry = deadStockRecord ? deadStockRecord.expiryDate : '';
    const lot = deadStockRecord ? deadStockRecord.lotNumber : '';

    // 期限状態を取得してクラスに追加 (input要素用)
    const expiryStatusClass = getExpiryStatus(expiry);
    ...
    const topRow = `<tr class="inventory-row"><td rowspan="2" class="col-action"><div class="action-buttons">${actionButtons}</div></td>
        ...
        <td class="right col-unitprice">${master.nhiPrice?.toFixed(4) || ''}</td><td class="col-expiry"><input type="text" class="expiry-input ${expiryStatusClass}" placeholder="YYYYMM" value="${expiry}"></td><td class="col-wholesaler"></td><td class="col-line"></td></tr>`;
...
function generateInputSectionsHtml(packageLedgers, yjUnitName = '単位', cache, yesterdaysTotal) {
    const packageGroupsHtml = (packageLedgers || []).map(pkgLedger => {
        ...
        // 理論在庫(JAN)の計算用変数
        let theoreticalJanStockForInput = 0;
        ...
        if (pkgLedger.masters && pkgLedger.masters.length > 0) {
            ...
            if (janPackInnerQty > 0) {
                ...
                // JAN単位の理論在庫を計算
                theoreticalJanStockForInput = parseFloat(totalJanStock.toFixed(2));
            }
        }
        let html = `
        ...
        html += (pkgLedger.masters || []).map(master => {
            ...
            // data-theoretical 属性を追加し、理論在庫を埋め込む
            const userInputArea = `
                <div class="user-input-area">
                    <div class="form-group">
                       <label>① 本日の実在庫数量（予製除く）:</label>
                        <span class="physical-stock-display" 
                           data-product-code="${master.productCode}"
                           data-theoretical="${theoreticalJanStockForInput}">0.00</span>
            ...
            // 保存済みデータがある場合はそれを使用。ない場合は理論在庫をプレースホルダーにセットして空欄。
            if (relevantDeadStock.length > 0) {
                ...
                finalInputTbodyHtml = createFinalInputRow(master, null, true, theoreticalJanStockForInput);
            }
...
function generateDeadStockReferenceHtml(deadStockRecords, cache) {
    ...
    const rowsHtml = deadStockRecords.map(rec => {
        ...
        // 期限の状態を取得してクラスに追加 (td要素用)
        const statusClass = getExpiryStatus(rec.expiryDate);
        ...
        return `
             <tr>
                ...
                <td class="center col-ref-expiry ${statusClass}">${rec.expiryDate || ''}</td>
